...
# cpt_correlations = [CPTCorrelation.andrus_2007, CPTCorrelation.robertson_2009, CPTCorrelation.hegazy_2006, CPTCorrelation.mcgann_2015, CPTCorrelation.mcgann_2018]
# vs30_correlations = [Vs30Correlation.boore_2011, Vs30Correlation.boore_2004]
# data_subsets = [DataSubset.only_old, DataSubset.new_and_old,  DataSubset.only_new]

cpt_correlations = [CPTCorrelation.andrus_2007]
vs30_correlations = [Vs30Correlation.boore_2004]
data_subsets = [DataSubset.only_old, DataSubset.new_and_old,  DataSubset.only_new]

progress_bar = tqdm(total=len(cpt_correlations)*len(vs30_correlations)*len(data_subsets))

labels_for_big_histogram = []
residuals_for_big_datasets = []
...
for cpt_correlation in cpt_correlations:
    for vs30_correlation in vs30_correlations:
        ...
        data_vs30_df_for_correlation_loop = data_vs30_df[(data_vs30_df["cpt_vs_correlation"] == cpt_correlation) &
                                    (data_vs30_df["vs30_correlation"] == vs30_correlation)]

        residuals_from_variations = []
        for data_subset in data_subsets:

            labels_for_big_histogram.append(f"{cpt_correlation}_{vs30_correlation}_{data_subset}")

            #print(f"Processing {cpt_correlation} and {vs30_correlation} for {data_subset} dataset")

            if data_subset == DataSubset.only_old:
                data_vs30_df_for_dataset_loop = data_vs30_df_for_correlation_loop[data_vs30_df_for_correlation_loop["record_name"].isin(record_names_in_old_dataset)]
            elif data_subset == DataSubset.only_new:
                data_vs30_df_for_dataset_loop = data_vs30_df_for_correlation_loop[~data_vs30_df_for_correlation_loop["record_name"].isin(record_names_in_old_dataset)]
            else:
                data_vs30_df_for_dataset_loop = data_vs30_df_for_correlation_loop

            vs30_from_data_and_model_df = data_vs30_df_for_dataset_loop.merge(model_vs30_df,left_on = "record_name", right_on="record_name_from_data", how="left")
            ...
            # residuals_for_big_datasets.append(vs30_from_data_and_model_df["ln_data_vs30_minus_ln_model_vs30"].values)

            residuals_dict[f"{cpt_correlation}_{vs30_correlation}_{data_subset}"] = vs30_from_data_and_model_df["ln_data_vs30_minus_ln_model_vs30"].to_list()
        ...
        single_correlation_hist_title = f"Log residuals in Vs30 between Foster et al. (2019) model\n" \
                                        f"and predictions from new CPT data using {cpt_correlation} and {vs30_correlation}\n"
        ...
        plt.xlabel("log residual")
        plt.ylabel("count")
        plt.title(single_correlation_hist_title)
    ...
    plt.figure()
    plt.hist(residuals_for_big_datasets, bins=100, label=labels_for_big_histogram, histtype='step',
             stacked=False, fill=False)
    plt.legend()
    plt.xlabel("log residual")
    plt.ylabel("count")
    plt.savefig(output_dir.parent / "all_residual_hist.png", dpi=500)
    plt.close()

# Dump dictionary to pickle file
with open(output_dir.parent / 'all_residuals.pkl', 'wb') as pickle_file:
    pickle.dump(residuals_dict, pickle_file)
